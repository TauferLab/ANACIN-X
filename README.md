...
Use `--accept-conda-tos` only if you agree to Anaconda's default channel Terms of Service. Without that option, Conda may ask you to run the `conda tos accept` commands manually before it can create or install packages from the default channels.
...
* **Conda asks for Terms of Service acceptance**: either re-run `./install_all.sh --accept-conda-tos` if you agree to those terms, or run the `conda tos accept --override-channels --channel ...` commands shown by Conda manually.
