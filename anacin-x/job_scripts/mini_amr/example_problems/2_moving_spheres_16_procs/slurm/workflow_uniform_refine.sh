...
job_script_trace_pack_procs=${job_scripts_root}/trace_uniform_refine_pack_procs.sh
job_script_trace_spread_procs=${job_scripts_root}/trace_uniform_refine_spread_procs.sh
...
do
    for n_refine_level in `seq 4 ${max_refinement_level}`;
    do
        echo "Launching jobs for: proc. placement = ${proc_placement}, max refinement ${n_refine_level}"
        runs_root=${results_root}/uniform_refine/proc_placement_${proc_placement}/n_refine_${n_refine_level}/
        ...
        do
            ...
            if [ ${proc_placement} == "pack" ]; then
                trace_stdout=$( sbatch -N1 ${job_script_trace_pack_procs} ${app} ${csmpi_config} ${n_refine_level}) 
            elif [ ${proc_placement} == "spread" ]; then
                trace_stdout=$( sbatch -N16 ${job_script_trace_spread_procs} ${app} ${csmpi_config} ${n_refine_level})
            fi
            ...
            #build_graph_stdout=$( sbatch -N1 ${job_script_build_graph} ${dumpi_to_graph_bin} ${dumpi_to_graph_config} ${run_dir} )
