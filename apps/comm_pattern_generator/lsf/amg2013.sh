...
do
	...
	if [ ! -f "$app_config" ]; then
	    python3 > ${debugging_path}/create_json_output.txt 2> ${debugging_path}/create_json_error.txt ${anacin_x_root}/apps/comm_pattern_generator/config/json_gen.py "amg2013" ${msg_size} ${n_iters}
	fi

	# Trace execution
        LD_PRELOAD=${pnmpi} PNMPI_LIB_PATH=${pnmpi_lib_path} PNMPI_CONF=${pnmpi_conf} mpirun -np > ${debugging_path}/trace_exec_output.txt 2> ${debugging_path}/trace_exec_error.txt ${n_procs} ${app_bin} ${app_config}
        mv dumpi-* ${run_dir}
