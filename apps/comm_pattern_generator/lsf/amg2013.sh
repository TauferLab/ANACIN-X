...
do
	...
	do
		...
		mkdir -p ${run_dir}
		app_config=${anacin_x_root}/apps/comm_pattern_generator/config/amg2013_msg_size_${msg_size}_niters_${iters}.json

		# Create app config if doesn't exist
		if [ ! -f "$app_config" ]; then
		    python3 > ${debugging_path}/create_json_output.txt 2> ${debugging_path}/create_json_error.txt ${anacin_x_root}/apps/comm_pattern_generator/config/json_gen.py "amg2013" ${msg_size} ${iters}
		fi

		# Trace execution
		LD_PRELOAD=${pnmpi} PNMPI_LIB_PATH=${pnmpi_lib_path} PNMPI_CONF=${pnmpi_conf} mpirun -np > ${debugging_path}/trace_exec_output.txt 2> ${debugging_path}/trace_exec_error.txt ${procs} ${app_bin} ${app_config}
		mv dumpi-* ${run_dir}
