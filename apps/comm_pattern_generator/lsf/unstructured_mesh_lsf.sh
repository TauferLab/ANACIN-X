...
# Trace execution
	    #n_procs_per_node=$((n_procs/n_nodes))
            if [ ${proc_placement} == "pack" ]; then
                ...
                trace_stdout=$( bsub -n ${n_procs} -R "span[ptile=${n_procs_per_node}]" -q ${queue} -W ${time_limit} -o ${debugging_path}/trace_exec_output.txt -e ${debugging_path}/trace_exec_error.txt ${job_script_trace_pack_procs} ${n_procs} ${app} ${config} )
	    ...
	    #echo "Starting Build Event Graph"
            ...
            build_graph_stdout=$( bsub -n ${n_procs} -R "span[ptile=${n_procs_per_node}]" -w "done(${trace_job_id})" -q ${queue} -W ${time_limit} -o ${debugging_path}/build_graph_output.txt -e ${debugging_path}/build_graph_error.txt ${job_script_build_graph} ${n_procs} ${dumpi_to_graph_bin} ${dumpi_to_graph_config} ${run_dir} )
	    ...
	    #echo "Starting Extract Slices"
            # Extract slices
            extract_slices_stdout=$( bsub -n ${n_procs} -R "span[ptile=${n_procs_per_node}]" -w "done(${build_graph_job_id})" -q ${queue} -W ${time_limit} -o ${debugging_path}/extract_slices_output.txt -e ${debugging_path}/extract_slices_error.txt ${job_script_extract_slices} ${n_procs_extract_slices} ${extract_slices_script} ${event_graph} ${slicing_policy} )
