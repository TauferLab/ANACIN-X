...
comm_pattern_job_script=${anacin_x_root}/apps/comm_pattern_generator/lsf/message_race.sh
...
do
    ...
    comm_pattern_run_stdout=$( bsub -n ${n_procs} -R "span[stripe]" -q ${queue} -W ${time_limit} -o ${debugging_path}/lsf_output.txt -e ${debugging_path}/lsf_error.txt ${comm_pattern_job_script} ${n_procs} ${n_iters} ${msg_size} ${n_nodes} ${run_idx} ${example_paths_dir} ${run_dir} ${nd_start} ${nd_iter} ${nd_end} ${impl} )
...
bsub -n ${n_procs} -R "span[stripe]" -w ${kdts_job_dep_str} -o compute_kdts_output.txt -e compute_kdts_error.txt ${compute_kdts_script} "${run_dir}/../" ${graph_kernel} --slice_dir_name "slices" -o "kdts.pkl"
