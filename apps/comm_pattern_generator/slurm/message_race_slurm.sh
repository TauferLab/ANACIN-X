...
do
    ...
    do
        ...
        comm_pattern_run_stdout=$( sbatch -N 1 -p ${queue} -J ${comm_pattern_run_name} -t ${time_limit} -n ${n_procs} ${comm_pattern_run_script} ${n_procs} ${msg_size} ${n_iters} ${proc_placement} ${run_dir} ${example_paths_dir} ${debug_dir} )
