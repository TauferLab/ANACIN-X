...
do
    ...
    runs_root=${results_root}/n_procs_${n_procs}/proc_placement_${proc_placement}/nd_neighbor_fraction_${nd_neighbor_fraction}/msg_size_${msg_size}/
    ...
    do
        ...
        comm_pattern_run_stdout=$( sbatch -N 1 -p ${queue} -J ${comm_pattern_run_name} -t ${time_limit} -n ${n_procs} ${comm_pattern_run_script} ${n_procs} ${msg_size} ${n_iters} ${proc_placement} ${nd_neighbor_fraction} ${run_dir} ${example_paths_dir} ${debug_dir} )
