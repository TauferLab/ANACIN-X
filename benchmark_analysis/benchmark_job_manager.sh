...
for run_idx in `seq -f "%03g" ${run_idx_low} ${run_idx_high}`; do
	...
	if [ "${scheduler}" == "lsf" ]; then
		comm_pattern_run_stdout=$( bsub -n ${n_procs} -R "span[ptile=$((n_procs_per_node+1))]" -q ${queue} -W ${time_limit} -o ${debugging_path}/lsf_output.txt -e ${debugging_path}/lsf_error.txt ${comm_pattern_run_script} ${n_procs} ${msg_size} ${n_iters} ${proc_placement} ${run_idx} ${run_dir} ${paths_dir} ${nd_start} ${nd_iter} ${nd_end} ${impl} ${comm_pattern} ${nd_neighbor_fraction} ${x_procs} ${y_procs} ${z_procs} )
...
if [ "${scheduler}" == "slurm" ]; then
	...
	compute_kdts_stdout=$( sbatch -N ${n_nodes} -p ${queue} -t ${time_limit} -n ${n_procs} --ntasks-per-node=$((n_procs_per_node+1)) -o compute_kdts_out.txt -e compute_kdts_err.txt --dependency=afterok:${kdts_job_dep_str} ${job_script_compute_kdts} ${n_procs_compute_kdts} ${compute_kdts_script} ${runs_root} ${graph_kernel} ${slicing_policy} ${paths_dir} )
	compute_kdts_job_id=$( echo ${compute_kdts_stdout} | sed 's/[^0-9]*//g' )
fi

if [ "${scheduler}" == "lsf" ]; then
	kdts_job_dep_str=$( join_by "&&" ${kdts_job_deps[@]} )
	compute_kdts_stdout=$(bsub -n ${n_procs} -R "span[ptile=$((n_procs_per_node+1))]" -w ${kdts_job_dep_str} -o compute_kdts_output.txt -e compute_kdts_error.txt -q ${queue} -W ${time_limit} ${job_script_compute_kdts} ${n_procs_compute_kdts} ${compute_kdts_script} ${runs_root} ${graph_kernel} ${slicing_policy} ${paths_dir} )
	compute_kdts_job_id=$( echo ${compute_kdts_stdout} | sed 's/[^0-9]*//g' )
fi
